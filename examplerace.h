...
public:
    ...
    explicit ExampleRace(QObject *parent = nullptr);
    //Прокидываем мьютекс
...
public:
    ...
    Controller(QMutex* m) : mut(m) {
        ...
        QObject::connect(&workerThread, &QThread::finished, race, &QObject::deleteLater); //По завершению потока удаляем экземпляр класса
        QObject::connect(this, &Controller::operate, race, &ExampleRace::DoWork); //Запускаем "тяжелый" метод
        QObject::connect(race, &ExampleRace::sig_Finish, this, &Controller::sig_WorkFinish); //Пробрасываем сигнал для отображения в GUI
